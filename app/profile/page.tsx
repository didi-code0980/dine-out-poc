...
import { Heart, Compass, User, Clock, Info } from "lucide-react"
  ...
  return (
    <div className="min-h-screen bg-[oklch(0.15_0.02_250)] pb-20">
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <Card className="border-white/10 bg-white/5 backdrop-blur mb-6">
          <CardContent className="p-6">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-16 h-16 rounded-full bg-[oklch(0.70_0.15_85)] flex items-center justify-center">
                <User className="w-8 h-8 text-[oklch(0.15_0.02_250)]" />
              </div>
              <div>
                <h2 className="text-xl font-bold text-white">{user.name || "Guest User"}</h2>
                <p className="text-white/60 text-sm">{user.email || "guest@dineout.com"}</p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/10">
              <div className="text-center">
                <p className="text-2xl font-bold text-[oklch(0.70_0.15_85)]">12</p>
        ...
        {/* Preferences Section */}
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-white mb-3 px-2">My Preferences</h3>
          <Card className="border-white/10 bg-white/5 backdrop-blur">
            <CardContent className="p-4 space-y-3">
              <div className="flex justify-between items-center py-2 border-b border-white/5">
                <span className="text-white/60">Favorite Cuisine</span>
                <span className="text-white font-medium">{preferences.cuisine || "Not set"}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-white/5">
                <span className="text-white/60">Price Range</span>
                <span className="text-white font-medium">{preferences.price || "$$"}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-white/60">Max Distance</span>
                <span className="text-white font-medium">{preferences.distance || "10"} miles</span>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        {/* Saved Spots Section */}
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-white mb-3 px-2">Saved Spots</h3>
        ...
        <div className="mt-8 mb-4 space-y-3">
          ...
          >
            ...
            <span className="text-sm">About Dine Out</span>
          </Link>
        </div>
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-[oklch(0.12_0.02_250)] border-t border-white/10 backdrop-blur">
        <div className="container mx-auto px-4 py-3 flex items-center justify-around max-w-2xl">
          ...
          <Link href="/profile" className="flex flex-col items-center gap-1 text-[oklch(0.70_0.15_85)]">
